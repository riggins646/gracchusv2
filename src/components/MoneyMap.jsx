...
export default function MoneyMap({
  ...
  const [viewMode, setViewMode] = useState("lens");
  const [lensSubjectId, setLensSubjectId] = useState(() => {
    // Default subject: first curated featured id. featuredIds is sorted by
  ...
  const [tierFilter, setTierFilter] = useState("AB"); // "AB" | "ABCD"
  const [minGBP, setMinGBP] = useState(1_000_000);    // min £ on award edges
  ...
  useEffect(() => {
    ...
    nodeSel.append("circle")
      ...
      .attr("stroke", (d) => d3color(TYPE_COLOUR[d.type]).brighter(0.4).formatHex())
    ...
    <div className="mm-root-container -mx-3 sm:-mx-6 -mt-4 sm:-mt-8 min-h-[900px] bg-[#060608] text-[#e5e7eb]">
      ...
      <section className="mm-filters">
        ...
        <Chip
          active
          onClick={() => {
            const steps = [100_000, 1_000_000, 10_000_000, 100_000_000];
            const idx = steps.indexOf(minGBP);
            setMinGBP(steps[(idx + 1) % steps.length]);
          }}
        >
          Min £: {fmtGBP(minGBP)}
        </Chip>
      ...
      <section className="mm-main">
        ...
        <aside className="mm-col mm-col-left">
          <div className="mm-rail-h">Story cards · featured this month</div>
          {(data.storyCards || []).map((s, i) => (
            <div
              key={i}
              className="mm-card"
              onClick={() => {
      ...
      <section className="mm-below mm-below-feature">
        <div className="mm-below-full">
          ...
          <p className="mm-feature-sub">
            ...
            Click any row to lens on that supplier.
          </p>
        ...
        <div className="mm-below-left">
          ...
          {(data.rankings?.topBuyersByHHI || []).slice(0, 8).map((b, i) => (
            ...
            >
              ...
              <span className="mm-rank-val">HHI {b.hhi.toFixed(2)}</span>
        ...
        <div className="mm-below-right">
          ...
          {(data.rankings?.topDependentSuppliers || []).slice(0, 8).map((s, i) => (
            ...
            >
              ...
              <span className="mm-rank-val">{fmtPct(s.dependence)}</span>
...
function Drawer({
  ...
  if (node) {
    ...
    return (
      <aside className="mm-drawer mm-drawer-open">
        <div className="mm-d-head">
          <button className="mm-d-close" aria-label="close" onClick={onClose}>×</button>
...
function DrawerScores({ node }) {
  ...
  if (node.kind === "buyer") {
    rows.push({
      ...
      hint: "Herfindahl-Hirschman Index across £-bearing suppliers. Higher = more concentrated. Computed only on suppliers with positive £ recorded in window.",
      citation: scores.buyerConcentrationHHI != null
        ? cite(`buyer concentration HHI ${scores.buyerConcentrationHHI.toFixed(2)}`)
        : null,
    });
    rows.push({
    ...
    rows.push({
      ...
      hint: "Share of this supplier's £ that came from its single largest buyer. 100% = single-buyer.",
      citation: scores.supplierDependence != null
        ? cite(`single-buyer dependence ${fmtPct(scores.supplierDependence)}`)
        : null,
    });
    rows.push({
      k: "Public buyers",
...
function MoneyMapStyles() {
  return (
    <style>{`
      ...
      .mm-ro-row {
        display: grid;
        grid-template-columns: 32px minmax(220px, 1fr) 60px auto;
